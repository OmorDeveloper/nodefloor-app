...
workspace also has messages of its own to deliver. This document is the contract for
...
| Name | Lives in | Holds |
|---|---|---|
| **MD queue** | NodeFloor (zustand store, per agent) | messages parked by *NodeFloor* until that agent's terminal is free |
| **Claude queue** | inside Claude Code itself | text Claude Code has accepted but not started on yet |

Everything below is about the **MD queue**. NodeFloor cannot see the Claude queue
...
- **Automation never closes your menu.** We do not send Escape at a picker. You may
  ...
  queued message is not NodeFloor's call, and we cannot verify that Escape worked
